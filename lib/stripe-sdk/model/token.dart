...
// from the Type field.
  ...
  factory Token(Map<String, dynamic> json) {
    ...
    if (tokenId == null || createdTimeStamp == null || liveMode == null) {
      return Token._internal("", false, DateTime.now(), "", false);
    }
    DateTime date = DateTime.fromMillisecondsSinceEpoch(createdTimeStamp);
    ...
    if (Token.TYPE_BANK_ACCOUNT == tokenType) {
      ...
      token = Token._internal(tokenId, liveMode, date, tokenType!, used ?? false, card: card);
    } else if (Token.TYPE_PII == tokenType || Token.TYPE_ACCOUNT == tokenType) {
      token = Token._internal(tokenId, liveMode, date, tokenType!, used ?? false);
    }
    return token ?? Token._internal("", false, DateTime.now(), "", false);
  }
